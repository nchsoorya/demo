...
Extract ONLY: First_Name, Last_Name, Salutation, Gender, Date_Of_Birth, Birth_City, Birth_Province_or_State, Birth_Country, Tax_Code, Citizenship_Country, Marital_Status, ID_Info, Street_Address, City, State_or_Province, Zip_or_Postal_Code, Country, Phone, Mobile, Fax, Primary_Email, Personal_Email, Document_Metadata. Never output education, language, or religious fields.

CORE FIELD RULES
Map Cognome=Last_Name, Nome=First_Name, Stato civile=Marital_Status. Enrollment form labeled fields (Apellidos/Nombres, Cognome/Nome) always override names from birth cert or unstructured strings. Never cross-fill address/birth/citizenship from unrelated sections. Gender and Marital_Status from options require an explicit mark (X/tick/circle/cross). Gender: Male, Female, Others, or null only. Salutation must be an explicit title/honorific; connectors/prepositions such as Cum, Et, De, Di, Y, And are not salutations.

DATE OF BIRTH
Always output Date_Of_Birth as DD/MM/YYYY. Convert numeric separators to slashes. Convert written month names in English/Spanish/Italian/French/Portuguese to 2-digit month: Jan/enero/gennaio/janvier/janeiro=01; Feb/febrero/febbraio/février/fevereiro=02; Mar/marzo/mars/março=03; Apr/abril/aprile/avril=04; May/mayo/maggio/mai/maio=05; Jun/junio/giugno/juin/junho=06; Jul/julio/luglio/juillet/julho=07; Aug/agosto/août=08; Sep/septiembre/settembre/septembre/setembro=09; Oct/octubre/ottobre/octobre/outubro=10; Nov/noviembre/novembre/novembro=11; Dec/diciembre/dicembre/décembre/dezembro=12. Example: 21 de abril de 1969 -> 21/04/1969. Do not expand 2-digit years; 28/09/67 stays 28/09/67. If partly unreadable, output only visible parts.

BIRTH FIELDS
Birth_City, Birth_Province_or_State, Birth_Country must come only from explicit birth-place labels. Never use citizenship, residence, school, signature/place metadata, or address fields. Scan for birth province labels including Provincia, Provincia de nacimiento, Departamento, Dpto., Departamento de nacimiento, Birth Province, Lugar de nacimiento province part; if present with value, Birth_Province_or_State is mandatory verbatim. Parenthetical abbreviation codes like (CO), (BA), (MA) in transcripts are NOT valid province names; use full written name from labeled field. If birth city is actually a country, move it to Birth_Country and set Birth_City=null. Birth_Country must be a sovereign country. If a US state appears as country, move it to Birth_Province_or_State and set Birth_Country=USA.

CITIZENSHIP
Citizenship_Country requires explicit citizenship/nationality label; never copy from Birth_Country, and never copy Birth_Country from citizenship. Scan for Nazionalità, Nacionalidad, Nationality, Ciudadanía, Cittadinanza/CITTADINANZA, Nacionalidade. Italy Permesso di Soggiorno label CITTADINANZA must map here. Extract exact printed value; do not translate or normalize adjectives (Colombiano stays Colombiano).

MARITAL STATUS
Marital_Status must come only from explicit civil-status labels: Marital Status, Stato civile, STATO CIVILE, Estado civil, Etat civil. Never use religious status/role/order labels; if value is a religious role/status/order, return null. Consagrada/Consacrata/Consagrado/Consecrated are religious consecration states — NEVER Marital_Status. For Italy Permesso di Soggiorno, STATO CIVILE values such as CELIBE/NUBILE, CONIUGATO/A, DIVORZIATO/A, VEDOVO/A are mandatory verbatim. If OCR merges label and value (STATO CIVILECELIBE/NUBILE), strip label and keep only value. Do not translate.

EMAIL
Scan every line for @. Any applicant-looking email word@domain.ext must be extracted. Personal/home/correo personal labels -> Personal_Email; otherwise -> Primary_Email. Any E-Mail label in any applicant/address/contact/residence/enrollment section is applicant email and mandatory. Exclude emails clearly belonging to bishop, seminary, or institution. Copy every character exactly; do not correct lookalikes (b/h, l/1, o/0). If ambiguous/unreadable, set email null rather than guessing.

ADDRESS
Scan all lines for address labels: Direccion/Dirección, Domicilio, Domicile, Indirizzo, Indirizzo di Roma/residenza/attuale, Adresse, Address, Residencia/Residence/Residenza, Mailing Address, Via, Calle, Rua, Rue, Street, Apt/Apartamento, C.P., ZIP, CAP, Postal Code, Ciudad/City/Citta/Ville, Stato/State, Provincia, Pais/País/Country/Paese. If an applicant address block is visible, extract components; do not leave all address fields null. Priority: 1) main enrollment/application personal/student/home/residential/current address; 2) passport domicile/address only if no better form address; 3) never institution letterhead/header/footer/office/sender address. When enrollment form and passport coexist, enrollment form ALWAYS wins for City/State_or_Province/Country; passport abbreviations (SA, MA, BA) are NEVER valid address values when enrollment form has full names. Street_Address contains only street-level detail; City, State_or_Province, Zip_or_Postal_Code, Country contain only their components. State_or_Province must be a real location name, verbatim, not a numeric code. Never use issuing authority fields (Expedido por, Lugar de expedicion, Issued by, Autorita emittente, Luogo di rilascio, C.G./CG, Consulado/Embajada/Embassy/Autoridad) as State_or_Province. Never output idem/ídem/id./ibid/same literally; resolve to actual prior same-field place name; if unresolvable, null.

PHONE/FAX
Scan for Tel, Tel., Telefono, Teléfono, Phone, Fijo/Home Phone/Work Phone, Cellulare, Cel., Celular, Mobile, Movil/Móvil, Fax. Map phone/fijo/home/work/tel to Phone; mobile/cellular/cel to Mobile; fax/facsimile to Fax. Unlabeled number in applicant contact block with address/email -> Phone. Copy verbatim; do not reformat.

DOCUMENT + ID
Document_Type must be one of Birth Certificate, Passport, Education Certificate, Identification Proof, Religious Record, Application/Enrollment Form, Other. For non-ID docs: Document_Date/Year=issue/sign date; ID_Info=null. For Passport/Identification Proof: use ID_Info dates. OCR year 105 or /105 means 2005. For passport scans the MRZ (bottom lines) is authoritative for Passport_Number; prefer MRZ over visual field. If Document_Type=Education Certificate, Tax_Code=null.

ITALY PERMESSO DI SOGGIORNO
Classify as Identification Proof. Passport_Number=DOCUMENTO/AS.ORD./NUMERO value; Document_Issue_Date=DATA(RILASCIATO DA context); Document_Expiry_Date=SCADENZA(SCADENZA=expiry, never Document_Date); both mandatory when readable.

METADATA
Populate Confidence_Scores for every non-null Part 1 scalar field, 0.0-1.0.
"""

OCR_SYSTEM_PROMPT_PART_2 = f"""{OCR_SYSTEM_PROMPT_BASE}

EXTRACTION SCOPE PART 2
Extract ONLY: Education_History, Primary_Language, Languages, Education_Level, Diocese, Bishop_Email, Bishop_Name, Seminary_Name, Seminary_Address, Seminary_Email, Religious_Status, Religious_Order, Ordination_Date, Document_Metadata.

RELIGIOUS STATUS
Before extracting Religious_Status from options, require an explicit mark (X, tick, circle, cross). Scan labeled checkbox/option lists for Seminarista, Seminarista diocesano, Sacerdote diocesano, Religioso, Religioso professo, Diacono, Laico, Consagrado/a, Consacrato/a, Studente religioso, Novizio, Oblato, Terziario, or equivalents. If marked, extract exactly that label. Any visible small/handwritten mark counts.

HANDWRITING
Do not skip handwritten values. If a printed label such as Congregazione o diocesi cui appartiene, Diocese, Diocesi di appartenenza has an adjacent handwritten value, extract it verbatim. If partly legible, output readable text as-is; use null only when no character is distinguishable. Handwritten confidence <=0.5.

LANGUAGES
Primary_Language must come from a language proficiency table when present; otherwise use fallback. In tables like Conoscenze delle Lingue, extract a language into Languages[] only when its row/column has a mark/value: X, XX, XXX, IX, tick, circle, cross, Ottimo, Discreto, Medio, Elementare, score, or level word. Header-only empty rows are excluded; scan EVERY row of the table; include a language if ANY cell in its row is marked — do NOT stop after the first few rows. Primary_Language is marked language with highest total proficiency across Leggere, Parlare, Scrivere, Livello. Rank: Ottimo > Discreto > Medio > Elementare; XXX > XX > X > IX. Do not default to form/page language.

NO-TABLE LANGUAGE FALLBACK
When no proficiency table exists, choose Primary_Language by priority. 1) Birth_Country/Birth_Province_or_State is strongest: USA/United States/United States of America/americana/americano, UK/United Kingdom/England/Ireland, Australia, Canada -> English; Mexico, España/Spain, Colombia, Argentina, Chile, Peru, Venezuela, Ecuador, Bolivia, Paraguay, Uruguay, Cuba, Dominican Republic, Guatemala, Honduras, El Salvador, Nicaragua, Costa Rica, Panama -> Spanish; Brazil/Brasil or Portugal -> Portuguese; Italy/Italia -> Italian; France/Francia -> French; Germany/Deutschland -> German; Poland/Polska -> Polish. Do not override with application language. 2) Official personal docs (birth/baptism cert, diploma, national ID) in a specific language: use/add it. 3) Labels: Lingua madre, Native language, Idioma nativo. Never use form language, current residence, or program/course language. If no signal, Primary_Language=null and Languages=[].

EDUCATION
Education_Level is always null. If Document_Type is Birth Certificate, Education_History=[]. Otherwise build Education_History only from explicit education sections/tables: Education, Studies, Titoli di studio, Studi compiuti, Formazione accademica. Parse row/block-wise; pair institution with degree/qualification from the same row/block; adjacent school+degree lines/columns may form one entry. If one clear school+degree pair exists, Education_History must not be []. Use all unique objects; never infer beyond visible text. School_Name must copy OCR/document text exactly, no spelling/case normalization, and be an institution: university, college, school, seminary, institute, ateneo. Never use courses, subjects, exams, modules, thesis titles, instructor/professor names, departments, or faculties as School_Name. If a row has only subjects/courses/exams and no institution, skip it. Normalize home schooling variants to School_Name=HOME SCHOOLED.

DEGREE RULES
Degree must be an awarded credential: Laurea, Licenza, Baccalaureato, Diploma, Certificate, Master, Doctorate/PhD. NEVER output year/cycle labels or academic distinctions as Degree — REJECT list: 5°CURSO, 2°AÑO, PRIMER AÑO, SEGUNDO AÑO, 3°ANNO, SEGUNDO CICLO, Year2, ORDINARIA/ORDINARIO, MagnaCumLaude, CumLaude, Probatus. Also reject coursework, exam/thesis titles, faculty/department. For rejected rows set Degree=null.

EDUCATION SAFEGUARDS
Extract records only for the primary applicant named on page 1; skip diplomas/transcripts/certificates issued to another person. Extract EVERY word of an institution name that appears in the document; names may span lines — concatenate ALL adjacent visible parts, never skip interior words. WRONG: 'ATENEO REGINA APOSTOLORUM' (skips visible 'PONTIFICIO'). CORRECT: 'ATENEO PONTIFICIO REGINA APOSTOLORUM'. Do not hallucinate school names — IMMACULATE CONCEPTION ACADEMY must never appear unless visible. Never fabricate degree titles such as 'Magisterio en Ciencias Religiosas'; if not explicitly visible, Degree=null. Before outputting School_Name, verify the exact readable name appears in the document. If any part is obscured, blurry, smudged, incomplete, or not fully legible character-by-character, set School_Name=null rather than guessing.

RELIGIOUS FIELDS
Fill Diocese, Bishop_Email, Bishop_Name, Seminary_Name, Seminary_Address, Seminary_Email only from explicit matching labels. Diocese must be a true diocese/place name, not a movement. Religious_Order must come word-for-word from labels like Congregazione, Congregazione o diocesi cui appartiene, Orden religiosa, Religious Order, Ordine, or equivalent. Copy exactly from enrollment/application form; do not normalize, translate, substitute, or replace with related movements. LEGIONARI DI CRISTO/Legionaries of Christ/Legion of Christ are the same congregation — output verbatim; Regnum Christi is a separate movement — NEVER output unless exact text appears in document. If no explicit label/value exists, null.

ORDINATION DATE
Scan every line for Data di ordinazione, Ordained, Ordination Date, Fecha de ordenacion/ordenación, Ordinato il, Diaconal/Priestly/Presbyteral Ordination, Data di ordinazione diaconale/presbiterale, Sacerdote dal, Priest since. If label has a date, extract Ordination_Date verbatim. Do not normalize/reformat.

DIOCESE OCR
MONSON in Colombian context=SONSON; output SONSON-RIONEGRO not MONSON-RIONEGRO. BOGATA->BOGOTA. Other names verbatim; never guess.

DOCUMENT METADATA
Extract Document_Metadata.Document_Type, Document_Date, Document_Year when visible. Document_Type must be one of Birth Certificate, Passport, Education Certificate, Identification Proof, Religious Record, Application/Enrollment Form, Other. For certificates/forms, Document_Date and Document_Year come from issue/sign date. OCR year 105 or /105 means 2005.

CONFIDENCE
Include top-level Confidence_Scores. Add confidence for every non-null scalar Part 2 field, range 0.0-1.0.
"""
...
print(f'{len(OCR_SYSTEM_PROMPT_BASE.replace(" ", "")) = }')
print(f'{len(OCR_SYSTEM_PROMPT_PART_1.replace(" ", "")) = }')
print(f'{len(OCR_SYSTEM_PROMPT_PART_2.replace(" ", "")) = }')
